function drawDistribution3D(promp, list,s_ref, varargin)
...
for i=reshape(interval',1,[])
    subplot(subplotInfo/2,subplotInfo2,cpt);
    
    if(flag_mean == 1)
        meanTraj =promp.PHI_mean*promp.mu_w;
        fig = visualisationShared3D(meanTraj, promp.PHI_mean*1.96*sqrt(diag(promp.sigma_w )), sum(promp.traj.nbInput), promp.meanTimes,  i, col1, fig,'vecX', [promp.meanInterval:promp.meanInterval:promp.meanTimes*promp.meanInterval], 'shaded',shaded);
        if(without==0)
            if(length(col2) ==3)
                for j = 1 : max(promp.traj.nbTraj,10)
                    fig(size(fig,2) + 1) =  plot3([promp.traj.interval(j):promp.traj.interval(j):promp.traj.totTime(j)*promp.traj.interval(j)],promp.traj.y{j}(1 + promp.traj.totTime(j)*(i-1) :promp.traj.totTime(j)*i), 'color', col2,'linewidth',0.5);hold on;
                end
            else
                for j = 1 : max(promp.traj.nbTraj,10)
                    fig(size(fig,2) + 1) =  plot3([promp.traj.interval(j):promp.traj.interval(j):promp.traj.totTime(j)*promp.traj.interval(j)],promp.traj.y{j}(1 + promp.traj.totTime(j)*(i-1) :promp.traj.totTime(j)*i), col2,'linewidth',0.5);hold on;
                end
            end
        end
        ...
        datG = size(fig,2);
        if(length(col1) ==3)
            fig(size(fig,2) + 1) =  plot3([promp.meanInterval:promp.meanInterval:promp.meanTimes*promp.meanInterval],meanTraj(1 + promp.meanTimes*(i-1):promp.meanTimes*i), 'color', col1,'linewidth', 2);
        else
            fig(size(fig,2) + 1) =  plot3([promp.meanInterval:promp.meanInterval:promp.meanTimes*promp.meanInterval],meanTraj(1 + promp.meanTimes*(i-1):promp.meanTimes*i), col1,'linewidth', 2);
        end
        set(gca, 'fontsize', 20);
        disG = size(fig,2);
