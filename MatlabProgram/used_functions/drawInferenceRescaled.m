...
for vff=interval
    ...
    if(ismember(vff,recoData))
        ...
        %%03/05/22
        realT = s_ref / test.totTime;
        realInt = [realT:realT:realT*test.totTime];
        nameFig(size(nameFig,2) + 1) = plot(realInt(1:test.nbData),test.partialTraj(1+ test.nbData*(vff-init):test.nbData*(vff-init+1)),'ok','linewidth',3);
        dnG = size(nameFig,2);
    end
