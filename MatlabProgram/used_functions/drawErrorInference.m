	...
	for vff=intervalPlot
        ...
        if(shaded==1)
            ...
            for tmp = 1:length(OtherPro)
                ...
                nameFig = visualisation(otherPrior{tmp}, sum(nbInput), infTraj.timeInf, vff, 'g', 'shaded', nameFig,[intervalInf:intervalInf:RTInf]);
                end
            end
            otherP = size(nameFig,2);

            nameFig = visualisation(prior - varPrior, sum(nbInput), infTraj.timeInf, vff, 'b', 'shaded', nameFig,[intervalInf:intervalInf:RTInf]);
            nameFig = visualisation(prior + varPrior, sum(nbInput), infTraj.timeInf, vff, 'b', 'shaded', nameFig,[intervalInf:intervalInf:RTInf]);

            nameFig = visualisation(prior, sum(nbInput), infTraj.timeInf, vff, 'b', 'shaded', nameFig,[intervalInf:intervalInf:RTInf]);
            ...
            nameFig = visualisation(posterior, sum(nbInput), infTraj.timeInf, vff, 'r', 'shaded', nameFig,[intervalInf:intervalInf:RTInf]);
