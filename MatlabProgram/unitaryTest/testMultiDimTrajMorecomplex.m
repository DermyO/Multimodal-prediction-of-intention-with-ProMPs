...
v =  linspace(5,15,200)'; %fonction à modéliser: y = x/2 + 5 de 5 à 10 en 100 points
t{1}.nbTraj =20;
t{1}.y = cell(1, t{1}.nbTraj);
for i=1:t{1}.nbTraj
   begin = randn(1);
   timeSup = round(rand(1)*10);
   t{1}.totTime(i) = 100 + timeSup ; %change the totalTime by adding a number between 0 to 10
   t{1}.y{i} = begin*ones(t{1}.totTime(i),1) + v(1:t{1}.totTime(i)) + 0.1*randn(1, t{1}.totTime(i))';
   t{1}.yMat{i} = [t{1}.y{i},sin(t{1}.y{i})*2+6];
   t{1}.y{i} = [t{1}.y{i};sin(t{1}.y{i})*2+6];%2D
   t{1}.alpha(i) = s_ref/t{1}.totTime(i);
...
for i=1:t{1}.nbTraj
   begin = randn(1);
   timeSup = round(rand(1)*10);
   t{2}.totTime(i) = 100 + timeSup ; %change the totalTime by adding a number between 0 to 10
   t{2}.y{i} = begin*ones(t{2}.totTime(i),1) + v(1:t{2}.totTime(i))*0.8 + 0.1*randn(1, t{2}.totTime(i))';
   t{2}.yMat{i} = [t{2}.y{i},t{2}.y{i}*3+4];
   t{2}.y{i} = [t{2}.y{i};t{2}.y{i}*3+4];%2D
   t{2}.alpha(i) = s_ref/t{2}.totTime(i);
...
%plot recoverData
drawRecoverData(t{1}, inputName,  'Interval', [1,2],'Specolor','m','namFig', 1);
drawRecoverData(t{2}, inputName, 'Interval', [1,2], 'Specolor','k','namFig', 1);
...
if(expAlpha ~= test{1}.alpha)
    expAlpha =test{1}.alpha; 
    infTraj = inference(promp, test{1}, M, s_ref, c, h, test{1}.nbData, expNoise, expAlpha);
    drawInferenceRescaled(promp,{'y=x/2 + 5', 'y=sin((x/2 + 5))*2 +6'}, infTraj, test{1},s_ref, 'Interval', [1,2])
end
