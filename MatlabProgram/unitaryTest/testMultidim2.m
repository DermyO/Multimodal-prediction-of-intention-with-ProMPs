%example where the periodic RBF models better the move.
...
nbInput(2) = 1;
M(1) = 20; %number of basis functions to represent nbInput(1)
M(2) = 20;
...
v =  linspace(50,100,100)'; %fonction à modéliser: y = x/2 + 5 de 5 à 10 en 100 points
t{1}.nbTraj =20;
t{1}.y = cell(1, t{1}.nbTraj);
for i=1:t{1}.nbTraj
   t{1}.y{i} = v + 0.1*randn(1, length(v))';
   for tt=1: size(t{1}.y{i},1)
       t{1}.y{i}(tt) = cos(t{1}.y{i}(tt));
   end
...
for i=1:t{1}.nbTraj
   t{2}.y{i} = v*0.2 + 0.1*randn(1, length(v))';
   for tt=1: size(t{2}.y{i},1)
       t{2}.y{i}(tt) = cos(t{2}.y{i}(tt));
   end
...
%plot distribution
drawDistribution(promp{1}, inputName,s_ref, 'col', 'c', 'shaded');
drawDistribution(promp{2}, inputName,s_ref, 'col', 'c', 'shaded');
