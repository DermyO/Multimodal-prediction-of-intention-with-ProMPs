...
for indTraj = 1:4
    ...
    for indTest=1: t{indTraj}.nbTraj
        ...
        infTraj = inference(promp, test{1}, M, s_bar, c, h, test{1}.nbData, expNoise, 1, 3);
        ...
        if(infTraj.reco ~= trial)
            ...
            if(displayOne==1)
                displayOne=0;
                %drawInferenceRescaled(promp,inputName ,infTraj, test{1},s_bar,'Interval', [1:3], 'shaded', 'recoData', [1:3]);
